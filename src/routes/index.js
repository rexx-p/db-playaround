...
router.post("/add-and-book-seat", async (req, res) => {
  try {
    ...
    const [isolationResult] = await Multiplex.sequelize.query('SELECT @@transaction_isolation as isolation_level', {
      type: Multiplex.sequelize.QueryTypes.SELECT
    });
    console.log(`🔒 [Transaction] Current isolation level: ${isolationResult.isolation_level}`);

    // Set isolation level to READ COMMITTED
    await Multiplex.sequelize.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
    console.log('🔄 [Transaction] Changed isolation level to: READ COMMITTED');
